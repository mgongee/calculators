...
/**
 * This is the main routing &cintroller class of the Planks calculator
 */
class PlanksController extends CalculatorController{
	
	
	protected function pageIndex() {
		global $T;
		
		$projects = ProjectManager::getAll('planks');
		$projectList = array();
	...
	protected function pageReport() {
		...
		if (isset($_POST['project_id'])) {
			...
			if ($reportType == "excel") {
				...
				$filename = 'planks_' . $projectId . '_' . time() . '.xls';
			/** For Excel 2007		
				...
				*/
				...
				$filename = 'planks_' . $projectId . '_' . time() . '.doc';
