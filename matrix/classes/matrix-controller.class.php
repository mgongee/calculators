...
/**
 * This is the main routing &cintroller class of the Matrix calculator
 */
class MatrixController extends CalculatorController{
	
	
	protected function pageIndex() {
		global $T;
		
		$projects = ProjectManager::getAll('matrix');
		$projectList = array();
	...
	protected function pageReport() {
		...
		if (isset($_POST['project_id'])) {
			...
			if ($reportType == "excel") {
				...
				$filename = 'matrix_' . $projectId . '_' . time() . '.xls';
			/** For Excel 2007		
				...
				*/
				...
				$filename = 'matrix_' . $projectId . '_' . time() . '.doc';
